...
class Item(models.Model):
    ...
    def transform(self, xsl_file):
        ...
        xsl = etree.parse(os.path.join(os.path.join(settings.STATIC_URL, 'xsl'), xsl_file))
    ...
    def neume_detail_transform(self, n):
        ...
        from django.conf import settings
        from lxml import etree, html
        xsl = etree.parse(os.path.join(os.path.join(settings.STATIC_URL, 'xsl'), 'neume_detail.xsl'))
        transform = etree.XSLT(xsl)
        try:
