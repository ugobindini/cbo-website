...
class MelodyGeneratorForm(forms.Form):
    ...
    pattern = forms.CharField(max_length=2048, label="Melodic pattern",
                            ...
                            required=True)
    repertory = forms.MultipleChoiceField(choices=[('0', 'Troubadour melodies (341)'), ('1', 'Gregorian chants (8971)')], widget=forms.CheckboxSelectMultiple, required=False)
    modes = forms.MultipleChoiceField(choices=[(str(n), n) for n in range(1, 9)] + [('0', 'Other')], widget=forms.CheckboxSelectMultiple, required=False)
